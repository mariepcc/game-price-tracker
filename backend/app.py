...
def get_unique_search_texts():
    unique_search_texts = db.session.query(GameResult.search_text).distinct().all()
...
def debug_db_path():
    ...
    abs_path = os.path.abspath(path.replace("sqlite:///", ""))
    return jsonify({"db_uri": path, "db_file_absolute_path": abs_path})
...
def get_games_results():
    ...
    results = (
        GameResult.query.filter_by(search_text=search_text)
    ...
    for result in results:
        ...
        game_dict[url]["priceHistory"].append(
            {"price": result.current_price, "date": result.created_at}
        )
...
def add_tracked_game():
    ...
    response = {"message": "Tracked game added successfully", "id": tracked_game.id}
    return jsonify(response), 200
...
def remove_game():
    ...
    tracked_game = TrackedGames.query.filter_by(name=name).first()
    if tracked_game:
        db.session.delete(tracked_game)

    db.session.commit()

    return jsonify(
        {
            "message": f"{len(games)} games removed from GameResult and tracked game removed from TrackedGames."
        }
    ), 200
...
@app.route("/remove-tracked-game", methods=["POST"])
def remove_tracked_game():
    game_id = request.json.get("id")
    game = TrackedGames.query.get(game_id)
    if game:
        db.session.delete(game)
        db.session.commit()
        return jsonify({"message": f"Tracked game '{game.name}' removed."}), 200
    return jsonify({"error": "Tracked game not found."}), 404
...
def update_tracked_games():
    ...
    for tracked_game in tracked_games:
        ...
        if price_dropped(tracked_game):
            latest_price, previous_price = get_prices(tracked_game.name)
            send_email(tracked_game.name, latest_price, previous_price)

    response = {"message": "Tracked games updated successfully", "games": game_names}
    return jsonify(response), 200


def get_prices(game_name):
    game_result = (
        GameResult.query.filter_by(search_text=game_name)
        .order_by(GameResult.created_at.desc())
        .limit(2)
    )
    return game_result[0].current_price, game_result[1].current_price


def price_dropped(game_result):
    result = (
        GameResult.query.filter_by(search_text=game_result.name)
        .order_by(GameResult.created_at.desc())
        .limit(2)
    )

    current_price = result[0].current_price
    previous_price = result[1].current_price
...
def send_email(name, new_price, previous_price):
    mail_message = Message(
        f"Attention! {name} is on discount!",
        sender=os.getenv("MAIL_USERNAME"),
        recipients=[os.getenv("MAIL_USERNAME")],
    )
    mail_message.body = f"🎮 Great news! The price of {name} in PS Store just dropped to {new_price / 100} zł from {previous_price / 100}  and it is currently on discount! Do not miss the deal 🤑"
